...
class TestTextExtractorSnapshots(unittest.TestCase):
    ...
    def _extract_in_mode(self, file_path: Path, ext: str, ocr: bool, lang: str, mode: str) -> str:
        if mode == "path":
            return tw.extract_text(input_data=file_path, extension=ext, pages=None, ocr=ocr, language_ocr=lang)
        if mode == "str":
            return tw.extract_text(input_data=str(file_path), extension=ext, pages=None, ocr=ocr, language_ocr=lang)
        if mode == "bytes":
            return tw.extract_text(input_data=file_path.read_bytes(), extension=ext, pages=None, ocr=ocr, language_ocr=lang)
